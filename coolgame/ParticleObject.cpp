...
void ParticleObject::init2(float partRadius_, float scaleVal_, glm::vec4 color_, ShaderShape* shaderShape_, ParticleShader* particleShadCreator_) {
    ...
    ParticleCreator pcreator(shaderShape->ctx.mesh.vertices, partRadius);
...
void ParticleObject::draw2(glm::mat4& VP, glm::mat4& P) {
    ...
    glEnableVertexAttribArray(particleShadCreator->shaders[partShadIndex].vertexVBO);
    glEnableVertexAttribArray(particleShadCreator->shaders[partShadIndex].colorVBO);
    glEnableVertexAttribArray(particleShadCreator->shaders[partShadIndex].radiusVBO);
